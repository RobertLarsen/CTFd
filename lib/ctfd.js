...
CTFd.prototype.start = function() {
    ...
    this.on('ready', function() {
        that.plantInterval = setInterval(_.bind(that.plantFlags_, that), 3000);
        that.checkInterval = setInterval(_.bind(that.checkFlags_, that), 500);
        that.plantFlags_();
    });
    ...
    _.each(this.services, function(service) {
        _.each(that.teams, function(team) {
            ...
            that.executer.plantFlag(flag, function(status, line) {
                console.log(team.name, service.name, status, line);
            });
        });
    });
};
